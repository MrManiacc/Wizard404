...
public class EntityRenderer {
	...
	public void renderMeshes(List<Mesh> meshes){
		for(Mesh mesh : meshes){
			prepareTexturedModel(mesh.getTexturedModel());
			prepareMesh(mesh);

			GL11.glDrawElements(GL11.GL_TRIANGLES, mesh.getTexturedModel().getRawModel().getVertexCount(),
					GL11.GL_UNSIGNED_INT, 0);
	...
	public void renderPlayer(Player player){
		...
		GL11.glDrawElements(GL11.GL_TRIANGLES, player.getModel().getRawModel().getVertexCount(),
				GL11.GL_UNSIGNED_INT, 0);
		unbindTexturedModel();
	}
	...
	private void prepareMesh(Mesh entity) {
		Matrix4f transformationMatrix = Maths.createTransformationMatrix(entity.getPosition(),
